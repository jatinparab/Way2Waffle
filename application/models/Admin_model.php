...
//                                $this->db->order_by('co.Order_id', 'DESC');
		...
		public function addcategoryasset($data){
			if($this->db->insert('assetCategories',$data)){
				return true;
			}else{
				return $this->db->error();
			}
		...
		public function deleteasset($data){
			$id=$data['id'];
			if($this->db->query("DELETE FROM `assets` WHERE id = $id")){
				return true;
			}else{
				return $this->db->error();
			}
		...
		public function deletecategoryasset($data){
			$id = $data['id'];
			if($this->db->query("DELETE FROM `assetCategories` WHERE id = $id")){
				$this->db->query("DELETE FROM `assets` WHERE category_id = $id");
				return true;
			}else{
				return $this->db->error();
			}
